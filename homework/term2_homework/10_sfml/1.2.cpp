...
struct Ball
{
    ...
    // Изменённый метод, ИСПОЛЬЗУЕТ ЦВЕТ ИЗ COLOR 
...
int main()
{
    ...
    while (window.isOpen()) 
    {
        ...
        while (window.pollEvent(event)) 
        {
            ...
            {
                // Меняем цвет всех выделенных шаров на случайный
                for (Ball& b : balls)
                {
                    if (b.isChoosen)
                    {
                        b.color = sf::Color(rand() % 256, rand() % 256, rand() % 256);
                    }
                }
            }
            ...
            if (event.type == sf::Event::MouseMoved && isSelecting) 
            {
                sf::Vector2f mousePosition = window.mapPixelToCoords({event.mouseMove.x, event.mouseMove.y});
                selectionRect.setSize(mousePosition - selectionStart);


                for (Ball& b : balls) 
                {
                    sf::Vector2f ballTopLeft = b.position - sf::Vector2f(b.radius, b.radius);
                    sf::Vector2f ballBottomRight = b.position + sf::Vector2f(b.radius, b.radius);

                    if (ballTopLeft.x >= selectionRect.getPosition().x &&
                        ballTopLeft.y >= selectionRect.getPosition().y &&
                        ballBottomRight.x <= selectionRect.getPosition().x + selectionRect.getSize().x &&
                        ballBottomRight.y <= selectionRect.getPosition().y + selectionRect.getSize().y)
                    {
                        b.isChoosen = true;
                    }
                }
            ...
            if (event.type == sf::Event::MouseButtonReleased && isSelecting) 
            {
                isSelecting = false;
            }
        }
